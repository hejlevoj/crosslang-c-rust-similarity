...
\textbf{The original tier labels were not exactly reproducible.} An earlier
release of this dataset shipped tier labels whose generating script had since
diverged. Regenerating them reproduces the aggregate distribution closely
(mean $0.826$ against a reported $0.823$) but agrees on only 89\% of individual
pairs, with the disagreement spread symmetrically across adjacent tiers --- a
pattern that threshold movement alone cannot produce, and that indicates the
per-pair scores differed. The labels released here are the regenerated ones,
and the raw scores are now distributed so that the question cannot recur.
